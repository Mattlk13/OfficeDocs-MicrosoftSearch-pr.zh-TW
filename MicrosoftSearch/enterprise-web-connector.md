---
title: Enterprise網站 Microsoft Graph連接器
ms.author: mecampos
author: mecampos
manager: umas
audience: Admin
...
- MET150
- MOE150
description: 針對 Microsoft 搜尋 設定Enterprise網站Graph連接器
ms.openlocfilehash: 909e06318019254dcecacd21988d68716e238bf5
ms.sourcegitcommit: d3fe9bf98485f9c7ec964a7967e8ca9fef7db8a5
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/23/2022
ms.locfileid: "65646710"
---
<!---Previous ms.author: monaray --->

<!-- markdownlint-disable no-inline-html -->

# <a name="enterprise-websites-microsoft-graph-connector"></a>Enterprise網站 Microsoft Graph連接器

Enterprise網站 Microsoft Graph連接器可讓您的組織 **從其內部對應網站** 為文章和內容編制索引。 設定連接器並從網站同步處理內容之後，使用者就可以從任何Microsoft 搜尋用戶端搜尋該內容。

> [!NOTE]
> 請閱讀Microsoft 365 系統管理中心文章 [**中的設定 Microsoft Graph 連接器**](configure-connector.md)一文，以瞭解一般連接器設定指示。

本文適用于設定、執行及監視Enterprise網站連接器的任何人。 它會補充一般設定程式，並顯示僅適用于Enterprise網站連接器的指示。 本文也包含 [疑難排解的相關](#troubleshooting)資訊。
...
若要連線到您的資料來源，請填入網站的根 URL、選取編目來源，以及您想要使用的驗證類型：無、基本驗證或 OAuth 2.0，[Azure Active Directory (Azure AD) ](/azure/active-directory/)。 完成這項資訊之後，請選取 [測試連線] 以驗證您的設定。

### <a name="url"></a>URL

使用 [URL] 欄位來指定您想要編目之網站的根目錄。 企業網站連接器會使用此 URL 作為起點，並遵循此 URL 中的所有連結進行編目。

> [!NOTE]
> 您可以在單一連線中為最多 20 個不同的網站 URL 編制索引。 在 [URL] 欄位中，輸入以逗號分隔的網站 URL (，) 。 例如，`https://www.contoso.com,https://www.contosoelectronics.com`。

### <a name="crawl-websites-listed-in-the-sitemap"></a>網站圖中列出的編目網站
...
> ![Enterprise Web 連線器的 [連線設定] 窗格螢幕擷取畫面。](media/enterprise-web-connector/connectors-enterpriseweb-connectionsettings-dynamicconfig-small.png)
...
此外，連接器現在支援對內部部署網站進行編目。 若要存取內部部署資料，您必須先安裝並設定連接器代理程式。 若要深入瞭解，請[參閱 Microsoft Graph連接器代理程式](./graph-connector-agent.md)。

針對您的內部部署網站，選取 **[代理** 程式] 作為編目模式，然後在 [**內部部署代理** 程式] 欄位中，選擇您稍早安裝並設定的Graph連接器代理程式。  
...
資源識別碼、用戶端識別碼和用戶端密碼值將取決於您如何為您的網站設定Azure Active Directory (Azure AD) 型驗證：
...
2. 如果您使用應用程式作為網站的識別提供者做為資源，而使用不同的應用程式來存取網站，則用戶端識別碼會是第二個應用程式的應用程式識別碼，而用戶端密碼將會是在第二個應用程式中設定的秘密。 不過，資源識別碼會是您第一個應用程式的識別碼。
    ...
    您不需要在此應用程式中設定用戶端密碼，但您必須在應用程式的 [ **應用程式角色** ] 區段中新增應用程式角色，稍後會指派給您的用戶端應用程式。 在下列螢幕擷取畫面中，您可以看到如何新增應用程式角色。
    ...
    * 編輯新的應用程式角色：
      ...
      設定資源應用程式之後，您必須建立用戶端應用程式，並藉由在用戶端應用程式的 API 許可權中新增上述設定的應用程式角色，為其授與存取資源應用程式的許可權。 
    ...
    * 新增許可權：
    ...
    複製頁面中顯示的用戶端密碼值，因為它不會再次顯示。 稍後使用來自此應用程式的應用程式識別碼做為用戶端識別碼、此應用程式中的秘密做為用戶端密碼，第一個應用程式的應用程式識別碼做為系統管理中心的資源識別碼。
    
    **Windows 驗證** 只能在代理程式模式中使用。 它需要使用者名稱、網域和密碼。 您必須在 [使用者名稱] **欄位中** 提供使用者名稱和網域，格式如下：domain\username 或 username@domain。 密碼必須在 [ **密碼] 字** 段中輸入。 針對Windows 驗證，提供的使用者名稱也必須是安裝代理程式之伺服器中的系統管理員。

## <a name="step-3a-meta-tag-settings"></a>步驟 3a：中繼標籤設定
...
## <a name="step-3b-add-urls-to-exclude-optional-crawl-restrictions"></a>步驟 3b：新增 URL 以排除 (選擇性編目限制) 

有兩種方式可以防止頁面編目：不允許在robots.txt檔案中編目頁面，或將它們新增至排除清單。

### <a name="support-for-robotstxt"></a>支援robots.txt

連接器會檢查您的根網站是否有robots.txt檔案，如果存在，它會遵循並遵循該檔案內找到的指示。 如果您不想讓連接器在您的網站上編目特定頁面或目錄，您可以在robots.txt檔案的 「不允許」宣告中呼叫這些頁面或目錄。
...
Enterprise網站連接器只支援每個人都能看見的搜尋權 **限**。 索引資料會出現在搜尋結果中，且組織中的所有使用者都可以看到。

## <a name="step-7-set-the-refresh-schedule"></a>步驟 7：設定重新整理排程

Enterprise網站連接器僅支援完整重新整理。 這表示連接器會在每次重新整理期間重新編目所有網站的內容。 若要確定連接器有足夠的時間來編目內容，建議您設定大型重新整理排程間隔。 我們建議您在一到兩周之間排程重新整理。
