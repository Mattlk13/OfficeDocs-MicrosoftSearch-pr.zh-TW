...
- MET150
- MOE150
description: 使用 Microsoft 建置的檔案共用、SQL、Confluence 等連接器，為內部部署內容編制索引的 Microsoft Graph 連接器代理程式概觀。
ms.openlocfilehash: 037f3944730b553a618d5c4d2be315ac50a49eb5
ms.sourcegitcommit: 6ff032e46055eacf0f7f77c753965b6433f50117
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 05/17/2022
ms.locfileid: "65442553"
---
# <a name="microsoft-graph-connector-agent"></a>Microsoft Graph連接器代理程式

使用內部部署連接器需要安裝 *Microsoft Graph連接器代理* 程式軟體。 它允許在內部部署資料與連接器 API 之間進行安全的資料傳輸。 本文會引導您安裝和設定代理程式。

## <a name="installation"></a>安裝

從 [https://aka.ms/GCAdownload](https://aka.ms/gcadownload) 下載最新版的 Microsoft Graph連接器代理程式，並使用安裝組態助理安裝軟體。 這裡[提供連接](./graph-connector-agent-releases.md)器代理程式軟體的版本資訊
...
##### <a name="step-2-upload-the-certificate-to-the-azure-portal"></a>步驟 2：將憑證Upload至Azure 入口網站
...
如果 「測試連線」動作在建立連線時失敗，並顯示錯誤：「請檢查使用者名稱/密碼和資料來源路徑」，即使提供的使用者名稱和密碼正確，也請確定使用者帳戶具有安裝連接器代理程式之電腦的互動式登入許可權。 您可以檢閱登 [入原則管理](/windows/security/threat-protection/security-policy-settings/allow-log-on-locally#policy-management) 的相關檔，以檢查登入許可權。 此外，請確定資料來源和代理程式電腦位於相同的網路上。

如果在建立連線時出現下列失敗訊息：「1011：Graph連接器代理程式無法連線或離線」，請登入安裝代理程式的電腦，並檢查其是否正在執行。 如果代理程式未執行，請啟動代理程式應用程式。 如果連線持續失敗，請確認在註冊期間提供給代理程式的憑證或用戶端密碼尚未過期，且具有必要的許可權。
